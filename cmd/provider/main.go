...
import (
	...
	apisCluster "github.com/m-gran/provider-provider-cisco-aci/apis/cluster"
	apisNamespaced "github.com/m-gran/provider-provider-cisco-aci/apis/namespaced"
	"github.com/m-gran/provider-provider-cisco-aci/config"
	"github.com/m-gran/provider-provider-cisco-aci/internal/clients"
	controllerCluster "github.com/m-gran/provider-provider-cisco-aci/internal/controller/cluster"
	controllerNamespaced "github.com/m-gran/provider-provider-cisco-aci/internal/controller/namespaced"
	"github.com/m-gran/provider-provider-cisco-aci/internal/features"
	"github.com/m-gran/provider-provider-cisco-aci/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for provider-cisco-aci").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-provider-cisco-aci"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-provider-cisco-aci",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped provider-cisco-aci APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced provider-cisco-aci APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped provider-cisco-aci controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced provider-cisco-aci controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped provider-cisco-aci controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced provider-cisco-aci controllers")
	}
